...
Ext.define('NAT.data.binding.Container', {
	...
	getRemoteStores: function(){
		var me = this;
		return Ext.Array.filter(me.stores.getRange(), function(store){
			return (store && store.proxy && store.proxy.api.read);
		});
	},

	load: function(op, callback, scope) {
		var me = this;
		var remoteStores = this.getRemoteStores();
		async.forEach(remoteStores, function(store, done){
...
Ext.define('natjs.overrides.Object', {
	...
	Ext.Array.findModel = function(array, attribute, value) {
		for (var i=0; array.length > i; i++){
			if (array[i] && (array[i].get(attribute) == value)) return array[i];
		}
		return null;
	};

	Ext.Array.findAllModels = function(array, attribute, value) {
		if (!array) return [];
		return Ext.Array.filter(array, function(item){ return (item.get(attribute) == value); });
	};
...
Ext.define('NAT.panel.Panel', {
	...
	validateFields: function(op, callback, scope) {
		op = op || {};
		var me = this;
		var fields = this.query('[isBindableField]');
