...
Ext.define('NAT.data.binding.BindableField', {
	...
	bindStore: function(store) {
		...
		this.on('blur', this.field_blur, this);
	...
	readValue: function () {
		...
		if (this.store){
			var model = this.store.getCurrModel();
			if (model){
	...
	writeValue: function () {
		...
		this.isChanging = true;
		this.store.currModel.set(this.dataField, this.getValue());
		this.isChanging = false;
	},

	field_blur: function(field, op) {
		if (this.validateOnBlur || this.validateOnChange) return; //UI validation

		if (!this.store) return;
		if (!this.store.hasModel()) return;

		this.store.currModel.ValidateField(this.dataField, function(err, data) {
			if (this.preventMark) return;
			var fieldErrors = this.store.currModel.errors.getByField(this.dataField);
			var isValid = (fieldErrors.length == 0);
			if (isValid) {
				this.clearInvalid();
			} else {
				this.markInvalid(fieldErrors[0].message);
			}
		}, this);
	}
});
...
Ext.define('NAT.form.field.Radio', {
	...
	writeValue: function () {
		...
		this.isChanging = true;
		this.store.currModel.set(this.dataField, this.propertyValue);
		this.isChanging = false;
	},
